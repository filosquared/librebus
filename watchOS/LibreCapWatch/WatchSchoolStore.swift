...
final class WatchSchoolStore: NSObject, ObservableObject, WCSessionDelegate {
    ...
    @Published private(set) var status = "Open LibreCap on your iPhone to sync."
    ...
    func requestLatest() {
        ...
        guard session.isReachable else {
            status = snapshot?.signedIn == true
                ...
                : "Open LibreCap on your iPhone to send school data."
            return
        }
        let id = UUID()
        requestID = id
        ...
        timeoutTask = Task { [weak self] in
            ...
            self?.finishRequest(id, status: "No reply yet. Open LibreCap on your iPhone and sync there.")
        }
        session.sendMessage([WatchSnapshot.requestKey: true], replyHandler: { [weak self] reply in
            Task { @MainActor in
                ...
                if reply[WatchSnapshot.contextKey] == nil {
                    self.finishRequest(id, status: "Open LibreCap on iPhone and refresh school data.")
                } else {
                    self.accept(reply)
    ...
    private func accept(_ context: [String: Any]) -> Bool {
        ...
        do {
            ...
            status = "Update LibreCap on both your iPhone and Watch."
        } catch {
