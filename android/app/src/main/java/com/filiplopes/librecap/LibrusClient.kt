...
class LibrusClient {
    ...
    fun login(username: String, password: String): StudentProfile {
        ...
        if (portal.code != 200) throw LibrusClientError("LibreCap could not start the Librus login flow.")
        ...
        if (loginJson.string("status") != "ok") {
            throw LibrusClientError("LibreCap could not complete the Librus login flow.")
        }
    ...
    private fun authorizationUrl(value: String): String {
        val url = try { URI("https://api.librus.pl/").resolve(value) } catch (_: Exception) { throw LibrusClientError("LibreCap could not complete the Librus login flow.") }
