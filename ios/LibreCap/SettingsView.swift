    ...
    var body: some View {
        Form {
            ...
            Section(settings.text(.notifications)) {
                Toggle(settings.text(.schoolNotifications), isOn: Binding(
                    get: { model.notificationsEnabled },
                    set: { enabled in Task { await model.setNotificationsEnabled(enabled) } }
                ))
                .disabled(model.isUpdatingNotifications)
                Text(settings.text(.schoolNotificationsDescription))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                if model.notificationPermission == .denied || model.notificationPermission == .restricted {
                    Label(settings.text(.notificationsBlocked), systemImage: "bell.slash")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
            }
