"""Single-window macOS launcher for Librebus."""
...
os.environ["LIBREBUS_SKIP_WIZARD"] = "1"
os.environ["LIBREBUS_CHECK_BROWSER"] = "0"
...
def show_initial_password() -> None:
	...
	script = f'display dialog "{escaped}" with title "Librebus" buttons {{"OK"}} default button "OK"'
...
def run_embedded() -> None:
	...
	threading.Thread(target=serve, name="librebus-server", daemon=True).start()
	if not ready.wait(timeout=15):
		raise RuntimeError("Librebus server did not start within 15 seconds")
	if startup_error:
		raise startup_error[0]

	show_initial_password()
	webview.create_window("Librebus", app_url(), width=1200, height=800, min_size=(900, 600))
